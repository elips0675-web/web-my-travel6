...
export default function Routes1Page() {
    ...
    const routeStats = useMemo(() => {
        if (!optimizedRoute || optimizedRoute.length === 0) {
            return { totalDistance: 0, totalMinutes: 0, fuel: 0 };
        }

        const totalMinutes = optimizedRoute.reduce((sum, p) => sum + (p.travelFromPrev?.minutes || 0), 0);
        const totalDistance = optimizedRoute.reduce((sum, p) => sum + parseFloat(p.travelFromPrev?.distance || 0), 0);
        
        const fuel = transportMode === 'car' ? (totalDistance / 100) * 8 : 0; // 8L/100km
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-3 pt-3 border-t space-y-1 text-sm">
                                                ...
                                                {transportMode === 'car' && routeStats.fuel > 0 && (
                                                    <div className="flex justify-between"><span className="font-bold">Примерный расход бензина:</span><span className="font-bold">{routeStats.fuel} л</span></div>
                                                )}
                                            </div>
                                        </div>
                                    </div>
